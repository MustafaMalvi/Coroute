  ...
  return (
    <div className="bg-white border border-ink/10 rounded-2xl shadow-sm overflow-hidden">
      ...
      <div className="p-5 bg-paper/60 flex flex-wrap gap-2">
        {(isRecurring ? ride.occursToday : (ride.status !== 'Cancelled' && new Date(ride.departureTime) > new Date())) && (
        ...
        {isRecurring ? (
          <>
            {ride.status !== 'Paused' ? (
              ...
              <button disabled={busy} onClick={handleResume} className={badge.route}>Resume</button>
            )}
            <button disabled={busy} onClick={handleCancelToday} className={badge.danger}>Cancel Today's Ride</button>
            <button disabled={busy} onClick={handleCancelTomorrow} className={badge.danger}>Cancel Tomorrow's Ride</button>
            <button disabled={busy} onClick={() => setRescheduleMode('today')} className={badge.route}>Reschedule Today's Ride</button>
            <button disabled={busy} onClick={() => setRescheduleMode('schedule')} className={badge.route}>Reschedule Entire Schedule</button>
          ...
          ride.status !== 'Cancelled' && new Date(ride.departureTime) > new Date() && (
            <>
              <button disabled={busy} onClick={handleCancelRide} className={badge.danger}>Cancel Ride</button>
              <button disabled={busy} onClick={handleDelete} className="text-xs font-bold text-white bg-alert-500 px-3 py-1.5 rounded-lg hover:bg-alert-600 transition-colors">Delete Permanently</button>
            </>
          )
        )}
    ...
    <div className={wrap.page}>
      <div className="max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
          <div>
            <span className={heading.eyebrowRoute}>Ride Host</span>
