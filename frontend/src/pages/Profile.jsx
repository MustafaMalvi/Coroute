...
import { field, spinner, wrap } from '../styles/style';
...
const ReviewRow = ({ review }) => (
  <div className="bg-paper p-4 rounded-xl border border-ink/10">
    <div className="flex items-center justify-between gap-3 mb-1.5">
      <span className="font-semibold text-sm text-ink">{review.reviewer?.name || 'A rider'}</span>
    ...
    <p className="text-[11px] text-ink/35 mt-1.5">{new Date(review.createdAt).toLocaleDateString()}</p>
  </div>
);
...
const Profile = () => {
  ...
  const [profileData, setProfileData] = useState({
    ...
    college: '', bio: '', avatarUrl: '', rating: 5, reviewCount: 0,
  ...
  useEffect(() => {
    fetchProfileData();
    fetchStats();
    if (user.role === 'host') fetchReviews();
  }, [user.token]);
  ...
  const fetchStats = async () => {
    try {
      ...
      // dashboard still works fine without these numbers, just skip it
    }
  };

  const fetchReviews = async () => {
    try {
      const res = await api.get(`/api/reviews/host/${user.id}`);
      setReviews(res.data);
    } catch (err) {
      // reviews are a nice-to-have on this page, not worth failing the load over
    }
  };
  ...
  if (loading) {
    return <div className="flex-1 flex justify-center items-center py-24"><div className={spinner}></div></div>;
  }
  ...
  return (
    <div className={wrap.page}>
      <div className="max-w-3xl mx-auto">
              ...
              <div className="mt-2 flex items-center justify-center sm:justify-start gap-2">
                ...
                {user.role === 'host' && (
                  <span className="text-paper/60 text-xs">
                    {profileData.rating.toFixed(1)} ({profileData.reviewCount} {profileData.reviewCount === 1 ? 'review' : 'reviews'})
                  </span>
                )}
              </div>
            </div>
              ...
              <form onSubmit={handleProfileUpdate} className="space-y-6 animate-fade-in">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  <div>
                    ...
                    <input type="text" value={profileData.name} onChange={(e) => setProfileData({ ...profileData, name: e.target.value })} className={field.input} required />
                  </div>
                  <div>
                    <label className={field.label}>GR / Student ID</label>
                    <input type="text" value={profileData.studentId} onChange={(e) => setProfileData({ ...profileData, studentId: e.target.value })} className={field.input} />
                  </div>
                  <div>
                    <label className={field.label}>Phone Number</label>
                    <input type="tel" value={profileData.phoneNumber} onChange={(e) => setProfileData({ ...profileData, phoneNumber: e.target.value })} className={field.input} />
                  </div>
                  <div>
                    <label className={field.label}>Gender</label>
                    <select value={profileData.gender} onChange={(e) => setProfileData({ ...profileData, gender: e.target.value })} className={field.input}>
                  ...
                  <div>
                    ...
                    <input type="text" value={profileData.college} onChange={(e) => setProfileData({ ...profileData, college: e.target.value })} className={field.input} />
                  </div>
                  <div>
                    <label className={field.label}>Date of Birth</label>
                    <input type="date" value={profileData.dob} onChange={(e) => setProfileData({ ...profileData, dob: e.target.value })} className={field.input} />
                  </div>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-2 border-t border-ink/10">
                  <div>
                    <label className={field.labelSpaced}>Emergency Contact Name</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className={field.labelSpaced}>Emergency Contact Phone</label>
                    <input
                      type="tel"
                ...
                {user.role === 'host' && (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-2 border-t border-ink/10">
                    ...
                    <div>
                      <label className={field.label}>Vehicle Model</label>
                      <input
                        type="text"
        ...
        {user.role === 'host' && reviews.length > 0 && (
          <div className="bg-white rounded-3xl shadow-sm border border-ink/10 p-6 sm:p-8 mt-6">
            <h3 className="font-display text-lg mb-4">What riders are saying</h3>
